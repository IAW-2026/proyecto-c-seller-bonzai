...
const publicRoutes = [
  "/", "/api/health", "/api/auth/check-email",
  "/api/user/activate-role", "/api/products/browse", "/api/sellers",
  "/sign-in", "/sign-up", "/webhooks", "/activate-seller",
];
...
export default clerkMiddleware(async (auth, req) => {
  ...
  if (pathname.startsWith("/activate-seller")) return NextResponse.next();

  if (publicRoutes.some((r) => pathname.startsWith(r))) return NextResponse.next();

  const serviceKey = req.headers.get("x-service-key");
  if (serviceKey === process.env.SERVICE_API_KEY) return NextResponse.next();
