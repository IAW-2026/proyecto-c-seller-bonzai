...
export async function POST(
  ...
  try {
    ...
    if (!seller.suspended) {
      return NextResponse.json({ error: "ALREADY_ENABLED", message: "Seller is already enabled." }, { status: 409 });
    }
    ...
    return NextResponse.json({ success: true, clerkId, action: "enabled" });
  } catch (error: any) {
    console.error("[admin/users/enable]", error);
