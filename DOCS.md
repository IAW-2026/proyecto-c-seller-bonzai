# Bonzai — Documentación detallada

> Documentación complementaria al README principal. Contiene arquitectura, referencia completa de la API, flujo de compra paso a paso y decisiones de diseño.
...
```
Buyer App (frontend compras) ──x-service-key──> Seller API
Payments App (procesa pagos) ──x-service-key──> Seller API
Shipping App (gestión envíos) <── dispatches + callback ── Seller Dashboard

Vendedor (navegador) ──Clerk auth──> Dashboard UI ──> Seller API
Admin (navegador) ──Clerk auth──> Admin UI ──> Seller API
```

### Apps externas

Este proyecto es **una de cuatro aplicaciones** que conforman el ecosistema Bonzai:

| App | URL | Rol |
|---|---|---|
| **Seller Dashboard** (esta) | `https://proyecto-c-seller-bonzai.vercel.app` | API REST + panel vendedor/admin |
| **Buyer App** | — (externa) | Frontend de compras: navega productos, reserva, crea órdenes, consulta tracking |
| **Payments App** | — (externa) | Procesa pagos y confirma órdenes via `POST /api/payments/confirm` |
| **Shipping App** | `https://proyecto-c-shipping-bonzai.vercel.app` | Gestiona envíos: recibe dispatches via `POST /api/shipping/dispatch` y llama de vuelta con el tracking ID |

### Flujo multi-vendedor

Cuando un comprador agrega productos de **distintos vendedores** a un mismo carrito:

1. **Reservas**: `POST /api/reservations/bulk` descuenta stock atómicamente y crea reservas (15 min de vencimiento) agrupadas por vendedor.
2. **Órdenes**: `POST /api/orders/new` agrupa las reservas por `sellerId` y crea **una Orden por vendedor**. Cada orden es independiente con su propio estado y sus propios OrderItems.
3. **Agrupación (Purchase)**: `POST /api/payments/confirm` recibe los `orderIds` de todas las órdenes del carrito (múltiples vendedores) y las agrupa bajo un solo **Purchase**. El Purchase representa el pago único del comprador que cubre múltiples vendedores.
4. **Envíos independientes**: cada vendedor envía su parte del pedido por separado desde su dashboard. Cada orden avanza `PAID` → `AWAITING_TRACKING` → `SHIPPED` de forma independiente.

### Integración con Shipping App (en desarrollo)

Actualmente, cuando un vendedor hace clic en **Ship** desde el dashboard:

1. El frontend envía `PATCH /api/orders/[id]` con `{ action: "ship" }`.
2. El servidor avanza la orden a `AWAITING_TRACKING`.
3. Como paso **fire-and-forget** (no bloqueante), envía un dispatch a `POST https://proyecto-c-shipping-bonzai.vercel.app/api/shipping/dispatch` con los datos del envío.
4. La Shipping App debe procesar el dispatch y llamar de vuelta a `POST /api/orders/[id]/tracking` con el `trackingId`.

**Nota**: esta integración está en una etapa intermedia. El dispatch se envía correctamente, pero el callback completo entre todas las apps (Buyer → Seller → Shipping → Seller → Buyer) se terminará de integrar en una tercera etapa del desarrollo.
---
